...
<?php
    ...
    if ($query == 'замок' || $query == 'любарта' || $query == 'замок любарта') $query = 1;
    if ($query == 'кірха' || $query == 'лютеранська' || $query == 'лютеранська кірха') $query = 2;
    if ($query == 'костел' || $query == 'святих' || $query == 'петра і павла') $query = 3;
    if ($query == 'покровська') $query = 4;
...
<?php
  function printResult ($result_set){
      ...
      $result_set = $mysqli->query("SELECT * FROM `arts` WHERE `id` LIKE '".$query."%'");
