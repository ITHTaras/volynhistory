...
<body>
    ...
    <div class="header d-flex flex-column flex-md-row align-items-center p-3 px-md-4 mb-3 bg-grey border-bottom box-shadow">
        ...
        <?php
        echo $_COOKIE['user'] ? "<a class='p-2 text-dark link'>Welcome, ".$_COOKIE['user']."</a>" : "";
        ?>
  ...
  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="text-align: center;">
        ...
        ?>
        <a class="p-2 text-dark link" href="/top.php">Top Places</a><br>
        <a class="p-2 text-dark link" href="/about.php">About</a><br>
        <?php
        if($_COOKIE['user']) { ?> 
            <a class="p-2 text-dark link" href="/add.php">Add Place</a>
        <br>
    ...
    <?php  } ?>
      ...
      <form method="POST" action="search.php?q=<?=$_POST["query"]?>" class="searchIt col-md-4 form-inline my-2 my-lg-0">
        <input name="query" class="search form-control mr-sm-2" type="search" placeholder="Lubart's Castle" aria-label="Search">
        <button name="submit" class="subSearch btn btn-outline-success my-2 my-sm-0" type="submit">Search</button>
